...
class ModelEvaluation:
    ...
    def _replace_zeros(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            raise MyException(e, sys)
    def _impute_missing_values_by_class(self, df: pd.DataFrame, target: pd.Series) -> pd.DataFrame:
        try:
            ...
            if target is None:
                raise Exception("Target column is required for class-based imputation.")

            for col in numeric_cols:
                if df_copy[col].isnull().sum() > 0:
                    df_copy[col] = df_copy.groupby(target)[col].transform(lambda x: x.fillna(x.mean()))
    ...
    def evaluate_model(self) -> EvaluateModelResponse:
        ...
        try:
            ...
            x = self._impute_missing_values_by_class(x,y)
